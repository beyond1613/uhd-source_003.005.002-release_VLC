...
int UHD_SAFE_MAIN(int argc, char *argv[]){
	...
	desc.add_options()
		...
		("bit-rate", po::value<double>(&bit_rate), "bit rate of PRBS")
		;
	po::variables_map vm;
	...
	uhd::sensor_value_t gps_gpgga = usrp->get_mboard_sensor("gps_gpgga");
	uhd::sensor_value_t gps_gprmc = usrp->get_mboard_sensor("gps_gprmc");
	std::cout << boost::format("%s") % gps_locked.to_pp_string() << std::endl;
	std::cout << boost::format("%s") % gps_time.to_pp_string() << std::endl;
	std::cout << boost::format("%s") % gps_gpgga.to_pp_string() << std::endl;
	std::cout << boost::format("%s") % gps_gprmc.to_pp_string() << std::endl;

	int numSamplePerSymbol = (rate / bit_rate);
	int count = 0;

	srand(time(NULL)); 
	double prbs[wave_table_len];
	for (size_t i = 0; i < wave_table_len; i++)
		prbs[i] = ((double)(rand()%2))/(1/ampl);
...
*/
			...
			buff[n] = std::complex<float>(prbs[index], 0);
		}
